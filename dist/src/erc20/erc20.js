...
import { Contract, num, RpcError, uint256, } from "starknet";
...
export class Erc20 {
    ...
    validateAmount(amount) {
        ...
        if (amountDecimals !== this.token.decimals) {
            throw new Error(`Amount decimals mismatch: expected ${this.token.decimals} (${this.token.symbol}), got ${amountDecimals}`);
        }
        // Only validate symbol if the amount has one set
        if (amountSymbol !== undefined && amountSymbol !== this.token.symbol) {
            throw new Error(`Amount symbol mismatch: expected "${this.token.symbol}", got "${amountSymbol}"`);
        }
    }
    /**
     ...
     */
    populateApprove(spender, amount) {
        this.validateAmount(amount);
        return this.contract.populateTransaction.approve(spender, uint256.bnToUint256(amount.toBase()));
    }
    /**
    ...
    populateTransfer(transfers) {
        return transfers.map((transfer) => {
            ...
            return this.contract.populateTransaction.transfer(transfer.to, uint256.bnToUint256(transfer.amount.toBase()));
        });
    }
    /**
     ...
     */
    async transfer(from, transfers, options) {
