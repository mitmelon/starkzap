import { RpcProvider, RpcError, TransactionFinalityStatus, } from "starknet";
 ...
 */
export async function checkDeployed(provider, address) {
    try {
...
function isContractNotFound(error) {
 ...
 */
export async function ensureWalletReady(wallet, options = {}) {
 ...
 */
export async function preflightTransaction(wallet, account, options) {
    ...
    try {
        ...
        if (result && "transaction_trace" in result && result.transaction_trace) {
            ...
            if ("execute_invocation" in trace &&
                ...
                if ("revert_reason" in invocation) {
                    return {
                        ok: false,
                        reason: invocation.revert_reason ?? "Simulation failed",
                    };
                }
            }
        }
...
export function sponsoredDetails(timeBounds, deploymentData) {
    return {
